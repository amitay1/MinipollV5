...
namespace NEEDSIM
{
    ...
    public class NEEDSIMNodeInspector : Editor
    {
        ...
        private string[] MakeInteractionOptions(DatabaseAsset data)
        {
            ...
            string[] result = new string[count + i];
            result[0] = firstInArray;

            if (data != null)
            {
                foreach (InteractionData interaction in data.Interactions)
                {
                    result[i] = interaction.interactionName;
                    i++;
                }
            }
        ...
        private void InteractionEditing(int loop)
        {
            ...
            {
                Debug.LogError("No interactions loaded for this needsim node. This can happen if the Needsim database in the scene does not match the one on a prefab.");
            }
            else
            {
...

        {
            ...
            selectedPlan.enumValueIndex = (int)(ExamplePlans)EditorGUILayout.EnumPopup("Behavior: ", (ExamplePlans)selectedPlan.enumValueIndex);
        ...
        private void StartNeedSatisfactionEditing()
        {
            ...
            for (int i = 0; i < serializedObject.FindProperty(StartingSatisfactionLevels_NeedKeys_arraySize).intValue; i++)
            {
                if(species == null)
                {
                    Debug.LogError("Null reference: species was null. This can happen if the Needsim database in the scene does not match the one on a prefab.");
                }
                else if(species.needs == null)
                {
                    Debug.LogError("Null reference: Species needs are null. This can happen if the Needsim database in the scene does not match the one on a prefab.");
                }
                else
                {
